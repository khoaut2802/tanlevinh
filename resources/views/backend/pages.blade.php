                ...
                <div class="rounded-lg shadow-lg py-6">
                    <div class="block overflow-x-auto mx-6">
                        <div class="float-right mb-2">
                            <a href="{{route('pages.edit')}}" class="rounded-md border border-transparent shadow-sm bg-green-700 text-white px-2">
                                Tạo trang
                            </a>
                        </div>                        
                            ...
                            <tbody>
                                @foreach($pages as $key => $page)
                                    <tr class="w-full font-light text-gray-700 bg-gray-100 whitespace-no-wrap border border-b-0">
                                        <td class="px-4 py-4">{{$page->id}}</td>                                    
                                        <td class="px-4 py-4">{{$page->title}}</td>
                                        <td class="px-4 py-4">{{$page->slug}}</td>
                                        <td class="px-4 py-4">
                                            <form method="POST" action="{{route('pages.delete', ['id' => $page->id])}}">
                                                @csrf
                                                <a href="{{route('pages.edit', ['id' => $page->id])}}" class="rounded-md border border-transparent shadow-sm bg-yellow-700 text-white px-2">Sửa</a>
                ...
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
                  ...
                  <div class="mt-2">
                    <div class="relative flex w-full flex-wrap items-stretch mb-3">
                        <input type="text" name="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Nhập tên Banner" required>
                    </div>
                    <div class="relative flex w-full flex-wrap items-stretch mb-3">
